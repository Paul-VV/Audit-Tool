...
def get_certification(takeoff,student):
    ...
    joined   = utils.str_to_time(student[JOINED], takeoff)
    solo     = utils.str_to_time(student[SOLO], takeoff)
    license_ = utils.str_to_time(student[LICENSE], takeoff)
    hours50  = utils.str_to_time(student[HOURS50], takeoff)
    ...
    if license_ is not None and takeoff >= license_:
...
def get_minimums(cert, area, instructed, vfr, daytime, minimums):
    ...
    conditions = 'VMC' if vfr else 'IMC'
    time_str   = 'Day' if daytime else 'Night'

    for row in data:
        category, cond, row_area, row_time = row[0], row[1], row[2], row[3]

        category_match = False
        if category == 'Dual' and instructed:
            category_match = True
        elif category == 'Student' and cert >= PILOT_STUDENT:
            category_match = True
        elif category == 'Certified' and cert >= PILOT_CERTIFIED:
            category_match = True
        elif category == '50 Hours' and cert == PILOT_50_HOURS:
            category_match = True
        ...
        area_match = (
            row_area == 'Any'
            or row_area == area
            or (row_area == 'Local' and area in ('Pattern', 'Practice Area'))
        )
        if not area_match:
            continue

        if row_time != time_str:
            continue
    ...
    ceiling     = get_best_value(matches, 4, maximum=False)
    visibility  = get_best_value(matches, 5, maximum=False)
    wind        = get_best_value(matches, 6, maximum=True)
    crosswind   = get_best_value(matches, 7, maximum=True)
